...
Bewertet nach Immocation Bierdeckelrechnung
Schreibt Ergebnisse in Google Sheet via Apps Script
Sendet Twilio SMS um 16:00 Uhr
"""
...
def send_sms(listings, config, dry_run=False):
    ...
    if dry_run:
              ...
              if listings:
                            top = listings[:3]
                            for l in top:
                                              logger.info(f"  - {l['title']}: {l.get('price',0):,.0f}€, "
                                                                                     f"Rendite normal {l.get('rendite_normal',0):.1f}% / "
                                                                                     f"WG {l.get('rendite_wg',0):.1f}% | Score: {l.get('gesamtscore',0)}")
                                      return True
...
def main():
    ...
    parser.add_argument('--dry-run', action='store_true',
                                               help='Testlauf ohne SMS/Sheet-Export')
    parser.add_argument('--limit', type=int, default=None,
                                               help='Max Listings pro Plattform (fuer Tests)')
    ...
    # Neue Listings filtern + bewerten
    new_listings = []
    ...
    # Nur interessante Listings (Score >= 30 oder Rendite >= 5%)
    interessante = [
    ...
    logger.info(f"Interessante Listings (Score>=30 oder Rendite OK): {len(interessante)}")
